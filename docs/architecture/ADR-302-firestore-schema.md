...
| 2026-05-04 | **v1.3 보강**: server-auth 권고 반영 — §1A `users/{uid}/fcmTokens/{tokenId}` 서브컬렉션 SSOT 신설(다중 디바이스 FCM 토큰). ADR-303 P1.1 정합. `users.notification.fcmToken` 단일 필드 deprecated → `notification.lastTokenAt` 디노멀 캐시로 대체. erd.md disk model + 관계 카디널리티 + 인덱스 2건 추가(JSON PASS). v1.0.0 시점 사용자 0이라 backfill 불필요. | server-data (server-auth 합의) |
